##  Initialize variables - SHARED VARIABLES ARE INCLUDED IN THE MALE FIGHT .rpy FILE

##=======ENTRY FUNCTIONS - 1 FOR EACH FIGHT CLASS=========

label ufc_female_outcome_d(bot):    ##  entry function for a D class fight
  $ufc_fight_level=3
  $ufc_bot=bot
  $ufc_mission_title="UFC Female D"
  $ufc_opponent=random.randint(1,2)
  call ufc_female_fight_scene
  return

label ufc_female_outcome_c(bot):    ##  entry function for a C class fight
  $ufc_fight_level=4
  $ufc_bot=bot
  $ufc_mission_title="UFC Female C"
  call ufc_female_fight_scene
  return

label ufc_female_outcome_b(bot):    ##  entry function for a B class fight
  $ufc_fight_level=5
  $ufc_bot=bot
  $ufc_mission_title="UFC Female B"
  call ufc_female_fight_scene
  return

label ufc_female_outcome_a(bot):    ##  entry function for an A class fight
  $ufc_fight_level=6
  $ufc_bot=bot
  $ufc_mission_title="UFC Female A"
  call ufc_female_fight_scene
  return

label ufc_female_outcome_s(bot):    ##  entry function for an S class fight
  $ufc_fight_level=7
  $ufc_bot=bot
  $ufc_mission_title="UFC Female Open"
  call ufc_female_fight_scene
  return

##=================DISPLAY FUNCTIONS=====================

label ufc_female_fight_scene:          ##  1st screen - bot fighting in arena
##  header already displayed by the 'mission_finished' function before calling bypass
  ...
  $action_image="missions ufc UFC_F_"+str(ufc_imagenumber)
...
##  "inside ufc_female_fight_scene, display an image and text showing the bot fighting"

  choice("ufc_female_determine_outcome") "Win or Lose?"
  return

label ufc_female_caught_cheating:                                ##  2nd screen if cheating, no room for avatar on this screen
...
##      $mc.give_xp("rep_mc_fighter",randint(-25,-10))    ## OLD CODE
    choice("ufc_female_buy_back") "Buy Bot Back"
    choice("ufc_female_do_not_buy_back") "Give Up Bot"
  else:
...
##      $mc.give_xp("rep_mc_trainer",randint(-25,-10))    ## OLD CODE
    if mc.rep_mc_fighter.level>=2:
      ...
      $mc.give_xp("rep_mc_fighter",temp)
##      $mc.give_xp("rep_mc_fighter",randint(-25,-10))    ## OLD CODE
    choice("ufc_female_do_not_buy_back") "Continue"
  return

label ufc_female_buy_back:                                       ##  3rd screen if cheating and buying back bot
  header "[ufc_bot.name] - {mark}[ufc_mission_title]{/} mission"
  $act.start_block("l:440 c:content_width-440")
  center "{image=bots [ufc_bot.model_id] avatar@240x360}"
  $act.set_block("c")
...
label ufc_female_do_not_buy_back:                                ##  3rd screen if cheating and NOT buying back bot
  header "[ufc_bot.name] - {mark}[ufc_mission_title]{/} mission"
  $act.start_block("l:440 c:content_width-440")
  center "{image=bots [ufc_bot.model_id] avatar@240x360}"
  $act.set_block("c")
...
label ufc_female_win_or_lose:                                    ##  2nd screen if not caught cheating, no room for avatar on this screen
  header "[ufc_bot.name] - {mark}[ufc_mission_title]{/} mission"
  ##  GRAPHICS
  $act.start_block("l:440 c:content_width-440")
  $act.set_block("l")
  if ufc_won_fight==1:                                                       ## your bot won
    ...
    ##  TEXT
    $act.set_block("c")
    "Yes! {mark}[ufc_bot.name]{/} wins the fight! Her record is now [ufc_bot.ufc_record]. {good}Hell yes! Prize money coming my way!{/}"
    ""
...
##    $mc.give_xp("rep_mc_fighter",randint(50,125))      ##  sexbot fighter reputation - 1st REVISION
    ...
    ##  TEXT
    $act.set_block("c")
    "Damn, {mark}[ufc_bot.name]{/} lost the fight. Her record is now [ufc_bot.ufc_record]. {bad}Shit, I could have used the money.{/}"
    ""
...
## LINE BELOW: bot won - enough wins - not too many losses
    ...
    choice("ufc_female_buy_offer") "Continue"
## LINE BELOW: bot qualified for offer but exceeds loss limit - AND you haven't called owner yet
    ...
    choice("ufc_female_call_owner") "Continue"
  else:                                              ## your bot lost
    choice("<<<") "Continue"
  return
...
## 0.12.n add a variable amount to the price based upon the MC's reputation  
  ...
  if ufc_bot.ufc_wins<ufc_high_offer:           ## INITIAL OFFER
    ...
    if ufc_bot.ufc_losses==0:                   ## bot is undefeated
      ...
      ""
      "That's a {mark}really good{/} offer, should I sell {mark}[ufc_bot.name]{/}? If [ufc_bot.heshe] keeps winning will it get better?"
  choice("ufc_female_sell_bot") "Sell Bot"
  choice("ufc_female_keep_bot") "Keep Bot"
  return

label ufc_female_sell_bot:
  header "[ufc_bot.name] - {mark}[ufc_mission_title]{/} mission"
  $act.start_block("l:440 c:content_width-440")
  center "{image=bots [ufc_bot.model_id] avatar@240x360}"
  $act.set_block("c")
...
label ufc_female_determine_outcome:    ##  determine if cheating, if not determine win or lose
  call ufc_female_evaluate_bot
  $temp_1=randint(1,100)
  if temp_1<ufc_bot_eval:            ##  random number less than bot evaluation, caught cheating
    call ufc_female_caught_cheating
...
##    $ufc_won_fight=1  ## force win for testing, comment out otherwise
    call ufc_female_win_or_lose
  return

label ufc_female_evaluate_bot:              ##  determines odds based upon bot and fight level
  python:
